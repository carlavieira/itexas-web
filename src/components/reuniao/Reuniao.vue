<template>
  <v-container>
    <v-row class="px-4">
      <h2>Reuniões</h2>
      <v-spacer></v-spacer>
      <v-btn @click="btnReuniao = true" title="Marcar Reunião" small color="secondary" fab>
    ...
    <v-row class="px-4 pt-3">
      <v-flex>
        <v-card class="w-100">
          ...
          <v-data-table no-data-text="Nenhuma reunião cadastrada" no-results-text="Nenhuma reunião encontrada" :headers="headersReuniao" :items="reuniao" :search="search"></v-data-table>
        </v-card>
      </v-flex>
    </v-row>
